...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0">تعديل عملية شراء</h2>
        ...
        <div class="card-body">
            <form action="{{ route('products.update', $product) }}" method="POST" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" min="0" name="wholesale_price" class="form-control" value="{{ old('wholesale_price', $product->wholesale_price) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">تكلفة المواصلات</label>
                    <input type="number" step="0.01" min="0" name="transport_cost" class="form-control" value="{{ old('transport_cost', $product->transport_cost) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">مصاريف إضافية</label>
                    <input type="number" step="0.01" min="0" name="extra_cost" class="form-control" value="{{ old('extra_cost', $product->extra_cost) }}">
                </div>
                <div class="col-12">
                    <button class="btn btn-primary">تحديث عملية الشراء</button>
                </div>
            </form>
        </div>
    </div>
